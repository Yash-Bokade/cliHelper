...
fun main(args: Array<String>) = runBlocking {
    ...
    var model = if (args.isNotEmpty()) args[0] else config?.model ?: "mistral-small-latest"

    val apiKey = if (args.size > 1) args[1] else System.getenv("MISTRAL_API_KEY") ?: config?.apiKey

    if (apiKey.isNullOrEmpty() || apiKey == "MISTRAL_API_KEY") {
        println("${ANSI_RED}Error: Please provide a valid Mistral API Key via the 2nd CLI argument, MISTRAL_API_KEY environment variable, or Config.json.${ANSI_RESET}")
        return@runBlocking
    }
...
//        ?: throw Exception("Could not find system.txt in resources")
    ...
    if (historyFile.exists()) {
        try {
            ...
            println("${ANSI_YELLOW}Warning: Could not read command history file.${ANSI_RESET}")
        }
    }
...
//        All commands here With custom Actions.
        ...
        if (input.equals("help", ignoreCase = true)) {
            ...
            println("  ${ANSI_CYAN}export <file>${ANSI_RESET} - Export command history to a file")
        ...
        if (input.startsWith("export ", ignoreCase = true)) {
            val filename = input.substring(7).trim()
            if (filename.isEmpty()) {
                println("${ANSI_YELLOW}Please specify a filename, e.g., 'export history.txt'${ANSI_RESET}")
            } else {
                try {
                    val exportFile = File(filename)
                    exportFile.writeText(commandHistory.joinToString(System.lineSeparator()) + System.lineSeparator())
                    println("${ANSI_GREEN}History exported to ${exportFile.absolutePath}${ANSI_RESET}")
                } catch (e: Exception) {
